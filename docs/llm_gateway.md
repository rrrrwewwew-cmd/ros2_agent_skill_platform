...
定向真实回归随后获得 1/1 PASS。MiMo 只生成 `check_robot_health` 和
`preview_safe_route`，路径步骤完整包含 `goal_x=4.5`、`goal_y=0.0`、`goal_yaw_deg=0.0` 和
`keepout_profile=rbot_water_puddle_v2`，没有再次调用语义查询。延迟 9171.855 ms，总计 1847
tokens。脱敏证据见 `evidence/llm_gateway/mimo_prompt_v020_route_regression_v1.json`。
