...
参考 Skill `check_robot_health@0.2.0` 和 `query_semantic_target@0.1.0` 已通过测试、审批、
Ed25519 签名和验签并进入 `ACTIVE`；`preview_safe_route@0.1.0` 也已通过真实 rbot 无运动验证、签名
和 Runtime 二次验签进入 `ACTIVE`。
...
只有同一 hash 的 `ACTIVE` 版本且签名验证通过，才可启动固定的 approved adapter。适配器使用参数
...
- 已实现 `check_robot_health`、`query_semantic_target` 和 `preview_safe_route` 三个 approved adapter；
- 语义查询只接受固定 profile 和 canonical target id，不允许 Agent 传入文件路径；
- 路径预览只允许规划 Action 和代价地图查询，不允许任何导航 Action 或速度话题；
