...
项目二已完成确定性安全底座、真实 MiMo 接入、Prompt 评测，以及只读 Agent Loop 的代码与集成
测试。生产 LLM 只使用 Xiaomi MiMo；`FakeProvider` 仅服务无网络 CI，不是第二个 API。当前 Agent
仍只执行只读 Skill，不触发审批，也不控制机器人。下一项是用正在运行的项目一仿真完成一次真实
MiMo `计划 → 健康检查 → 安全路径预览 → 证据 → Trace` 现场闭环。
...
1. 正常启动项目一 rbot 仿真栈，运行一次真实 MiMo 两步只读 Agent Loop；
2. 冻结父结果、两个子结果、状态事件和 JSONL Trace 证据；
