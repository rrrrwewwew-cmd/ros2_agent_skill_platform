...
v0.2.0 和 Gateway 本地校验中修复，真实定向回归 1/1 PASS。下一阶段已确定为只读 Agent Loop。
...
1. 实现有界只读 Agent Loop：计划 → 输入校验 → Runtime 调用 → 证据 → Trace；
2. 用 Fake Provider 完成确定性多步闭环测试，再用 MiMo + 仿真运行一次真实只读闭环；
...
# 定向真实回归已经完成；检查冻结证据
python3 -m json.tool \
  evidence/llm_gateway/mimo_prompt_v020_route_regression_v1.json
```
