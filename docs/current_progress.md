...
项目二已完成确定性安全底座、真实 MiMo 接入、Prompt 评测，以及只读 Agent Loop 的代码、集成
测试和 rbot 现场闭环。生产 LLM 只使用 Xiaomi MiMo；`FakeProvider` 仅服务无网络 CI，不是第二个
API。当前 Agent 仍只执行只读 Skill，不触发审批，也不控制机器人。下一主线进入版本化 RAG：先
冻结知识源和检索契约，再建立可复算检索评测；受控导航暂不暴露给模型。

- 工作区：`/home/li/robot_agent_ws`
- Git 分支：`feature/skill-registry-state-machine`
- 前四个 Skill：均已签名并处于 `ACTIVE`
- ROS 2 包：8 个
- 当前测试基线：156 项，0 error、0 failure、0 skipped
...
1. 建立 `robot_rag` 最小垂直切片：版本化 source manifest、内容 hash、chunk 契约和本地索引；
2. 首批只摄取 ROS 2 Jazzy/Nav2 官方文档快照、项目一公开接口说明和本仓库 Skill 契约；
3. 实现带 `source_id/version/hash` 引用的混合检索，并冻结最小检索评测集；
4. 完成 RAG 基线后接入 MCP 实验诊断工具，再进入 RAG-assisted Skill Author；
5. 受控导航只有在传感器基线和暂停/审批/恢复状态契约完成后才向模型暴露。
