...
当前发布状态为 `ACTIVE`。它已完成真实 rbot 往返仿真、显式人工发布批准、Ed25519 签名、
Runtime 二次验签，以及绑定完整 invocation 的一次性执行批准验证。
...
- 真实往返导航均未进入 Keepout，终点位置误差分别为 0.044 m 和 0.044 m；
- 正式 Runtime run `navigate_home_active_001` 为 `SUCCEEDED`，批准被原子消费一次，Trace 13 个事件。

完整冻结证据位于 `evidence/navigate_to_approved_pose/`。

## 仿真验收与受治理执行顺序

第一次真实仿真已按 Skill 作者测试路径完成。后续回归仍应先运行 `preview_safe_route`，将它输出
的新鲜 `route.path_sha256` 与 `keepout.source_content_sha256` 填入命令。直接运行固定 ROS adapter
只允许用于隔离作者仿真，不属于正常 Agent 执行路径：
